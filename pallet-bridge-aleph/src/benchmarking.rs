...
use sp_runtime::{DigestItem, traits::{One, Zero}};
use codec::Encode;

use crate::{BridgedHeader, AlephJustification};
use crate::pallet::{Config, Call, Pallet, ImportedHeaders, CurrentAuthoritySet};
use bp_aleph_header_chain::aleph_justification::test_utils::{Seeds, generate_authority_set, generate_justification};
use bp_aleph_header_chain::{AuthoritySet, ChainWithAleph, ConsensusLog, InitializationData, ALEPH_ENGINE_ID};
...
fn prepare_authority_change(count: u8) -> DigestItem {
	DigestItem::Consensus(
		ALEPH_ENGINE_ID, 
		ConsensusLog::AlephAuthorityChange(generate_authority_set(seeds_for_authority_change(count))).encode()
	)
}
...
fn prepare_authority_change_header<T: Config>(new_authorities_count: u8) -> BridgedHeader<T> {
    ...
    header.digest_mut().logs.push(
        prepare_authority_change(new_authorities_count)
    );
    header
}

fn prepare_initialization_data<T: Config>(initial_authorities_count: u8) -> InitializationData<BridgedHeader<T>> {
...
fn prepare_benchmark<T: Config>(initial_authorities_count: u8, new_authorities_count: u8) -> (BridgedHeader<T>, AlephJustification) {
	...
	let justification = generate_justification(&header.hash().clone().encode(), &initial_seeds(initial_authorities_count));
